// ECO opening database — common openings mapped by move sequence
// Moves are in SAN format joined by spaces

const ECO_DATABASE: { moves: string; name: string; eco: string }[] = [
...
// Sort by move sequence length (longest first) for longest prefix match
ECO_DATABASE.sort((a, b) => b.moves.length - a.moves.length);

export function lookupOpening(moveSans: string[]): { name: string; eco: string } | null {
